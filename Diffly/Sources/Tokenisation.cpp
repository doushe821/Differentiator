...
int FindVariable(const char* string, const VariableTable_t* VarTable);
...
void PrintTokens(Token_t* tokens, size_t number, VariableTable_t* VarTable)
{
    ...
    for(size_t i = 0; i < number; i++)
    {
        ...
        {
            fprintf(stderr, "Node№ %zu:\nType = %d\nCode = %d\nVarName = %s\n\n", i, tokens[i].type, tokens[i].Data.Decimal, VarTable->Vars[tokens[i].Data.Decimal].name);
        }
...
Token_t GetFunctionToken(const char* Buffer, size_t* index, __attribute((unused)) VariableTable_t* VarTable)
{
    ...
    {
        int code = FindVariable(funcName, VarTable);
        if(code != -1)
        {
            tok = {VARIABLE_TOKEN, code};        
        }
        else
        {
...
int FindVariable(const char* string, const VariableTable_t* VarTable)
{
    for(int i = 0; i < VarTable->Index; i++)
    {
        if(strncmp(string, VarTable->Vars[i].name, strlen(VarTable->Vars[i].name)) == 0)
        {
            return i;
        }
    }
